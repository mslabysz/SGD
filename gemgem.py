...
def runGame():
    ...
    while True: # main game loop
        ...
        remaining = max(0, int(timeLimit - elapsed))
        drawTimer(remaining)
        if remaining == 0 and not gameIsOver:
            gameIsOver = True
...
def drawTimer(remainingSeconds):
    timerSurf = BASICFONT.render(f"Time left: {remainingSeconds}s", True, SCORECOLOR)
    timerRect = timerSurf.get_rect()
    timerRect.midtop = (WINDOWWIDTH // 2, 5)
